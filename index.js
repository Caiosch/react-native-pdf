...
export default class Pdf extends Component {
  static propTypes = {
    ...
    source: PropTypes.oneOfType([
      PropTypes.shape({
        ...
        uniqueIdForCache: PropTypes.string,
  ...
  componentDidUpdate(prevProps) {
    ...
    const nextUniqueIdForCache = this.props.source.uniqueIdForCache;
    const curUniqueIdForCache = prevProps.source.uniqueIdForCache;

    if (
      nextSource.uri !== curSource.uri ||
      nextUniqueIdForCache !== curUniqueIdForCache
    ) {
  ...
  _loadFromSource = (newSource) => {
    let source = Image.resolveAssetSource(newSource) || {};
    let uri = source.uri || '';
    let uniqueIdForCache = source.uniqueIdForCache || '';
    let fileId = `${uri}${uniqueIdForCache}`;
    ...
    let cacheFile = RNFetchBlob.fs.dirs.CacheDir + '/' + SHA1(fileId) + '.pdf';
  ...
  _prepareFile = async (source) => {
    try {
      if (source.uri) {
        ...
        let cacheFile =
          RNFetchBlob.fs.dirs.CacheDir + '/' + SHA1(fileId) + '.pdf';
        ...
        this._onError(
          new Error('É preciso informar uma origem para o arquivo!'),
        );
      }
    } catch (e) {
      this._onError(e);
      this._onErrorComplete(e);
    }
  };
  ...
  _downloadFile = async (source, cacheFile) => {
    ...
    let tempCacheFile = cacheFile + '.tmp';
        ...
        if (
          ...
          try {
            ...
            if (!fileStats || !fileStats.size) {
              throw new Error('Arquivo não encontrado: ' + source.uri);
            }

            actualContentLength = fileStats.size;
          } catch (error) {
            throw new Error('Falha no download: ' + source.uri);
          }
  ...
  setPage(pageNumber) {
    if (pageNumber === null || isNaN(pageNumber)) {
      throw new Error('O número de página especificado não é um número.');
    }
    this.setNativeProps({
      page: pageNumber,
    });
    ...
    if (message.length > 0) {
      ...
      if (message[0] === 'loadComplete') {
        ...
        this.props.onPageSingleTap &&
          this.props.onPageSingleTap(message[1], message[2], message[3]);
        ...
        <View style={[this.props.style, { overflow: 'hidden' }]}>
          {!this.state.isDownloaded ? (
            ...
            <PdfView
              ...
              onErrorComplete={this._onErrorComplete}
...
if (Platform.OS === 'android') {
  var PdfCustom = requireNativeComponent('RCTPdf');
} else if (Platform.OS === 'ios') {
  var PdfCustom = requireNativeComponent('RCTPdfView');
}
