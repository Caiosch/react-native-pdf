...
export default class PdfView extends Component {
    ...
    this._scrollTimer = setTimeout(() => {
      if (this._flatList) {
        this._flatList.scrollToIndex({
          animated: false,
          index: this.props.page < 1 ? 0 : this.props.page - 1,
        });
      }
    }, 200);
  }
  ...
  _getPageWidth = () => {
    ...
    switch (fitPolicy) {
      ...
      case 1: //fit height
        return (
          this.state.contentContainerSize.height *
          this.state.pageAspectRate *
          this.state.scale
        );
      case 2: //fit both
      default: {
        if (
          this.state.contentContainerSize.width /
            this.state.contentContainerSize.height <
          this.state.pageAspectRate
        ) {
          return this.state.contentContainerSize.width * this.state.scale;
  ...
  _onScaleChanged = (pinchInfo) => {
    ...
    let newContentOffset = {
      x:
        (this.state.contentOffset.x + pinchInfo.pageX) *
          (newScale / this.state.scale) -
        pinchInfo.pageX,
      y:
        (this.state.contentOffset.y + pinchInfo.pageY) *
          (newScale / this.state.scale) -
        pinchInfo.pageY,
    };
  ...
  _getItemLayout = (data, index) => ({
    length: this.props.horizontal
      ...
      : this._getPageHeight(),
    offset:
      ((this.props.horizontal ? this._getPageWidth() : this._getPageHeight()) +
        this.props.spacing * this.state.scale) *
      index,
    index,
  });
  ...
  _onListContentSizeChange = (contentWidth, contentHeight) => {
    if (
      this.state.contentOffset.x != this.state.newContentOffset.x ||
      this.state.contentOffset.y != this.state.newContentOffset.y
    ) {
      this._flatList.scrollToXY(
        this.state.newContentOffset.x,
        this.state.newContentOffset.y,
      );
    }
  };
  ...
  _renderList = () => {
    ...
    if (this.props.singlePage) {
      data[0] = {
        key: this.props.currentPage >= 0 ? this.props.currentPage : 0,
      };
    } else {
      ...
      <PdfViewFlatList
        ...
        contentContainerStyle={[
          ...
          this.props.horizontal
            ? {
                height:
                  this.state.contentContainerSize.height * this.state.scale,
              }
            : {
                width: this.state.contentContainerSize.width * this.state.scale,
              },
        ]}
